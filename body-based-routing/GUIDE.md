...
4. **Multi-region**: Split traffic across multiple GKE clusters with RayServices, to attain higher
...
No InferencePool or Endpoint Picker is required, as Ray manages this for us.

## Example: Body-Based Routing for Ray Serve on GKE
...
TODO: These YAML must be inlined for a dev blog - the height seems to be limited
so the post should not be unwieldly.
...
- Check out recent Ray Serve performance optimizations: [Major upgrades to Ray Serve: Online Inference with 88% lower latency and 11.1x higher throughput](https://www.anyscale.com/blog/ray-serve-inference-lower-latency-higher-throughput-haproxy)
- Read more about Gateway on GKE: [About Gateway API](https://docs.cloud.google.com/kubernetes-engine/docs/concepts/gateway-api)
